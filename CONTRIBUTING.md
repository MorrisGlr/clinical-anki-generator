...
This installs the `cast` CLI in editable mode along with `pytest` and `pytest-cov`.
...
pytest --cov=cast --cov-report=term          # with coverage
```
...
4. Run `pytest` and `ruff check cast/` before pushing. CI will also run these checks.
...
Each parser lives in `cast/parsers/<platform>.py` and must expose:

- `parse(content: str, file_path: str) -> list[ParsedQuestion]`
- `SYSTEM_PROMPT: str`

Use `try_selectors()` and `try_patterns()` from `cast/core.py` for all CSS and regex lookups -- do not call `soup.find()` or `re.search()` directly. Add a synthetic HTML fixture to `tests/fixtures/` and corresponding tests to `tests/test_<platform>.py`.
